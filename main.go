...
var googleDomains map[string]string = map[string]string{
...
func randomUserAgent() string {
	...
	randNo := rand.Int() % len(userAgents)
	return userAgents[randNo]
}
...
func buildGoogleURLs(searchTerm, countryCode, languageCode string, pages, count int) ([]string, error) {
	...
	if googleBase, found := googleDomains[countryCode]; found {
		...
		err := fmt.Errorf("country (%s) isnt supported", countryCode)
		return nil, err
	}
	return toScrape, nil
}
...
func main() {
	res, err := GoogleScrape("Keshav Mundhra Crypto", "com", "en", nil, 1, 30, 10)
	fmt.Println("xxx")
	if err == nil {
		for _, re := range res {
			fmt.Println(re)
